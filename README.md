...
想在 30 秒內看到這個專案的核心行為，到「個股研究 → 手動輸入數據」填入下面這組
**虛構的**示範數值（不是任何真實公司的財報數字）：
...
- 表單即時顯示「已填權重 **90** / 需 60 才能評級」——權重表與計分規則是同一份來源
- 流動比率的卡片是**虛線邊框 + 問號圖示**，寫著「本項 10 分權重已排除於評分之外（未計零分）」
  —— 而不是打叉當成不及格
- 分數標註「在 **90** 分可評權重上正規化」，並列出「未納入評分：流動比率」

把流動比率填上 `1.6` 再跑一次，分數會從 79 變成 76 —— 因為那一項確實不理想。
**「不知道」與「知道它很糟」是兩件事**，這正是重構前被 `parseFloat(v || '0') || 0`
抹掉的那個區別。

> 示範數值僅供操作演示，不代表任何真實標的，也不構成投資建議。
...
靜態站台，任何支援 Vite 的平台都能直接部署，全部使用預設值：
...
不需要 SPA rewrite 設定 —— 本應用以狀態切換頁面而非 URL 路由，只有一個進入點。
